...
const adminService = {
  ...
  getAdminSetting: async (id) => {
    ...
    return {
      status: 'success',
      message: 'get admin info succeed',
      admin: user,
    };
  },
  ...
  getProduct: async (id) => {
    const product = await Product.findByPk(id, {
      ...
      order: [[ProductImage, 'is_display', 'DESC']],
    });
    if (product !== null) {
      return {
        status: 'success',
  ...
  putProduct: async (id, productInfo, productImages) => {
    ...
    try {
      ...
      const numDestroyedImages = await ProductImage.destroy({
        where: { product_id: id },
        transaction,
      });
  ...
  deleteProduct: async (id) => {
    ...
    try {
      ...
      const numDestroyedProduct = await Product.destroy({
        where: { id },
        transaction,
      });
    ...
    for (const key in salesLastYear) {
      if (salesLastYear.hasOwnProperty(key)) {
        ...
        const percentageChange =
          (percentChange(lastYear, thisYear) * 100).toFixed(2) + '%';
