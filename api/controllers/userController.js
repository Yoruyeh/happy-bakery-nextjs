...
const userController = {
  ...
  signUp: async (req, res, next) => {
    try {
      const { firstName, lastName, gender, email, password, termsAgreement } =
        req.body;
      // Error: required items
      if (!email || email.trim() === '')
        throw new CError('email is required', 400);
      if (!validator.isEmail(email))
        throw new CError('email is invalidated', 400);
      if (!password || password.trim() === '')
        throw new CError('password is required', 400);
      // Error: limitation
      if (firstName.length > 50 || lastName.length > 50)
        throw new CError('exceed word limit', 400);
      if (!['male', 'female', 'other', ''].includes(gender))
        throw new CError('gender input error', 400);
      // Error: agreement checkbox
      if (!termsAgreement) throw new CError('must agree to it', 400);

      const { status, message, token, user } = await userService.signUp(
        firstName,
        lastName,
        gender,
        email,
  ...
  getUserSetting: async (req, res, next) => {
    try {
      ...
      if (!req.isAuthenticated())
        throw new CError('User not authenticated', 401);
      if (req.user.isAdmin === true) throw new CError('Admin not allowed', 400);

      const currentUserId = req.user.id;
      const { status, message, user } = await userService.getSetting(
        currentUserId
      );
  ...
  putUserSetting: async (req, res, next) => {
    try {
      ...
      const currentUserId = req.user.id;
      const data = req.body;
      const { status, message, user } = await userService.putSetting(
        currentUserId,
        data
      );
      return res.json({
  ...
  putUserPassword: async (req, res, next) => {
    try {
      ...
      const { status, message } = await userService.putPassword(
        user.id,
        req.body
      );
  ...
  getUserOrders: async (req, res, next) => {
    try {
      ...
      const { status, message, userOrders } = await userService.getUserOrders(
        user.id
      );
