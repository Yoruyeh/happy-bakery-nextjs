...
const adminController = {
  signIn: async (req, res, next) => {
    try {
      ...
      const { status, message, token, user } = await adminService.signIn(
        email,
        password
      );
      return res.json({
  ...
  getAdminSetting: async (req, res, next) => {
    try {
      ...
      if (!req.isAuthenticated())
        throw new CError('Admin not authenticated', 401);
      if (req.user.isAdmin !== true) throw new CError('Admin only', 400);

      const currentAdminId = req.user.id;
      const { status, message, admin } = await adminService.getAdminSetting(
        currentAdminId
      );
      return res.json({ status, message, admin });
    } catch (error) {
      next(error);
    }
  },
  ...
  putPassword: async (req, res, next) => {
    try {
      const currentAdminId = req.user.id;
      const data = req.body;
      const { status, message, admin } = await adminService.putPassword(
        currentAdminId,
        data
      );
      return res.json({
  ...
  getProducts: async (req, res, next) => {
    try {
      ...
      const { status, message, productCount, products } =
        await adminService.getProducts(category, page, sort, keyword);
  ...
  postProduct: async (req, res, next) => {
    try {
      ...
      if (!validProduct(productInfo))
        throw new CError('invalid product info', 400);
      if (!validateImages(productImage))
        throw new CError('invalid product image', 400);

      const { status, message, newProduct, newImages } =
        await adminService.postProduct(productInfo, productImage);
      return res.json({
        status,
        message,
        newProduct,
  ...
  putProduct: async (req, res, next) => {
    try {
      ...
      const { status, message } = await adminService.putProduct(
        id,
        productInfo,
        productImage
      );
  ...
  getOrders: async (req, res, next) => {
    ...
    try {
      const { status, message, orderCount, orders } =
  ...
  putOrder: async (req, res, next) => {
    try {
      ...
      if (!isValidateId(id)) throw new CError('product id invalid', 400);
      if (!['pending', 'canceled', 'delivered'].includes(orderStatus))
        throw new CError('status invalid', 400);

      const { status, message } = await adminService.putOrder(
        id,
        orderStatus,
        note
      );
  ...
  getStatusSales: async (req, res, next) => {
    try {
      ...
      const { status, message, data } = await adminService.getStatusSales(
        startDate,
        endDate
      );
  ...
  getIntervalSales: async (req, res, next) => {
    try {
      ...
      const { status, message, data } = await adminService.getIntervalSales(
        year
      );
      res.json({ status, message, data });
    } catch (error) {
